Feature: Test Sign up form from Contact Us page with negative scenario where all fields are filled with incorrect credentials
    ...
    Scenario: Verify that "This field is required." message appears and user is not signed in after submitting Sign up form empty fields, the "Sign up" form is triggered after clicking the "portal" button on the "SMS API" page.
